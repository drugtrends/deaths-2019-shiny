...
#Might need this???: https://stackoverflow.com/questions/42169380/shiny-renderui-with-multiple-inputs
  ...
  observe({
    ...
    #   tagList(selectInput("codDT", "Intent:",
  ...
  output$opioidPlotD <- renderPlotly({
    ...
    sub$sex_intent <- paste(sub$sex,sub$intent,sep=",")
      ...
      p <- ggplot(sub) + aes(x = year, 
        ...
        scale_x_continuous(breaks = function(x) unique(floor(pretty(x)))) +
        labs(title=paste0(input$stateOD,", ",input$ageOD))
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
# All drugs plot (NOT USED)----------------------------------
#deleted this section from Tim's version because it is not used
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #

  #   # All drug deaths plot (NOT NEEDED - just a basic plot of the next)--------------------------------------------------------
  ...
  #         ),
  #         legend = list(y = 0.95, yanchor = "top"), #, x= 1.0, xanchor = "centre"), margin = list(b = 100, l = 100, r=200),
  ...
  #              toImageButtonOptions = (padding = list(r = 400))              )
  #     })
  ...
  output$allPlotB <- renderPlotly({
    #Based on: https://shiny.rstudio.com/reference/shiny/1.0.4/renderUI.html  
    if (input$DropAllB == "Intent") {
      sub <- filter(df, age_group %in% input$ageAllB & drug == "Drug induced deaths" & jurisdiction == input$stateAllB &
                                  intent == input$codAllI & sex %in% input$sexAllBI &
      ...
      sub <- subset(df, subset = (age_group %in% input$ageAllB & drug == "Drug induced deaths" & jurisdiction == input$stateAllB &
                                    intent %in% input$codAllS & sex == input$sexAllBS &
                                    (year >= input$yearsAllB[[1]] & year <= input$yearsAllB[[2]]) ) )
      sub$age_intent <- paste(sub$age_group,sub$intent,sep=",")
...
#Based on: https://shiny.rstudio.com/reference/shiny/1.0.4/renderUI.html  
    if (input$DropDT == "Age_Intent") {
      sub <- subset(df_DT, subset = (intent==input$codDT & nature=="Underlying" & age_group == input$ageDT & 
                    sex == "All" & jurisdiction == "AUS" & (year >= input$yearsDT[[1]] & year <= input$yearsDT[[2]] )
                    & drug %in% input$drugDT ) )
    ...
    if (input$DropDT == "Drug") {
      sub <- subset(df_DT, subset = (intent %in% input$codDT & nature=="Underlying" & age_group %in% input$ageDT & 
                       sex == "All" & jurisdiction == "AUS" & (year >= input$yearsDT[[1]] & year <= input$yearsDT[[2]]) ) &
                       drug == input$drugDT )
      sub$age_intent <- paste(sub$age_group,sub$intent,sep=",")
      p <- ggplot(sub) + aes(x = year, colour = age_intent, linetype = age_intent, group = 1) +
        geom_line() +
        labs(x = "Year") +
        theme_light() + scale_colour_manual(values = agecodcols) +
...
#      layout(legend = list(orientation = "h", y = -0.15, yanchor = "top"), margin = list(b = 100, l = 100)) %>% 
  ...
  output$PlotOE <- renderPlotly({
    ...
    p <- ggplot(sub) + aes(x = year, colour = drug, linetype = age_group, group = 1) +
        labs(x = "Year") + geom_line() + theme_light() + theme(legend.title = element_blank()) +
        scale_colour_manual(values = drugcols) + scale_linetype_manual(values = agetype) +
        scale_x_continuous(breaks = function(x) unique(floor(pretty(x))))
